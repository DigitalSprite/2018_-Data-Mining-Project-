class PrefixSpan:
    ...
    def __init__(self, sequences, minSupport=2, maxPatternLength=10):
                ...
                for item in element:
                    if item not in counted:
                        ...
                        if item in items:
                            ...
                            items[item] = 1
        f_list.extend([self.SequencePattern([[self.PLACE_HOLDER, k]], _items[k], maxPatternLength, self.PLACE_HOLDER)
                       for k in _items
                       if _items[k] >= threshold])
        f_list.extend([self.SequencePattern([[k]], items[k], maxPatternLength, self.PLACE_HOLDER)
                       for k in items
...
#     ]
#
#     model = PrefixSpan.train(sequences, minSupport=, maxPatternLength=5)
