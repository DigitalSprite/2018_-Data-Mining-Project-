...
def get_union_feature():
    data = pd.read_csv('../../a/data/type1/count/data_u_b.csv')
    new_data = pd.read_csv('../../a/data/type1/count/data_u.csv')
    data = pd.merge(data, new_data, how='left')
    new_data = pd.read_csv('../../a/data/type1/count/data_b.csv')
    data = pd.merge(data, new_data, how='left')
    new_data = pd.read_csv('../../a/data/type1/diversity/data_u_b.csv')
    data = pd.merge(data, new_data, how='left')
    new_data = pd.read_csv('../../a/data/type1/penetration/data_b.csv')
    data = pd.merge(data, new_data, how='left')
    new_data = pd.read_csv('../../a/data/type2/month_agg/count/data_b.csv')
    data = pd.merge(data, new_data, how='left')
    new_data = pd.read_csv('../../a/data/type2/month_agg/count/data_u_b.csv')
    data = pd.merge(data, new_data, how='left')
    ...
    data = pd.merge(data, new_data, how='left')
    new_data = pd.read_csv('../../a/data/type2/month_agg/penetration/data_b.csv')
    data = pd.merge(data, new_data, how='left')
    new_data = pd.read_csv('../../a/data/type2/user_agg/user_agg_b.csv')
    data = pd.merge(data, new_data, how='left')
    ...
    temp_data = []
    for index, t in new_data.groupby(['vipno', 'bndno']):
        temp_data.append(list(index))
    temp_data = pd.DataFrame(temp_data, columns=['u', 'b'])
    temp_data['tag'] = 1
    data = pd.merge(data, temp_data, how='left').fillna(0)
...
if __name__ == "__main__":
    ...
    plt.title('The ROC diagram of user_brand_features')
